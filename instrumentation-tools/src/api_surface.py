...
DIVIDER = "#"
TESTABLE_API_REG_EX = re.compile("^(.+)(constructor|fun)(.+)")
ROOT_REG_EX = re.compile("^(?<!companion)(\\s*)[a-zA-z]*(class|interface|enum|object)(.*)(\\.|\\s)(.+)$")
...
def sanitize_prefix(previous_indent: int,
    ...
    difference = floor((current_indent - previous_indent) / 2)
    if difference <= 0:
...
class ApiSurface:
    ...
    def fetch_testable_methods(self) -> set:
        ...
        current_indent = 0
        for row in rows:
            is_group_root_matcher = ROOT_REG_EX.match(row)
            ...
            elif TESTABLE_API_REG_EX.match(row) and (
